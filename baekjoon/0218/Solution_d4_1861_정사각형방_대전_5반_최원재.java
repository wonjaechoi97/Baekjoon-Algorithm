import java.io.*;
import java.util.*;
public class Solution_d4_1861_정사각형방_대전_5반_최원재 {
	...
	//dfs활용
	static void solve(int i, int j, int currentNum, int cnt,int[][] map, int N) {
		if(currentNum+1!=map[i][j])return;
		
		for(int d=0;d<4;d++) {
			if(1<=i+di[d]&&i+di[d]<=N&&1<=j+dj[d]&&j+dj[d]<=N) {
				solve(i+di[d],j+dj[d], map[i][j], cnt+1, map, N);
			}
		}
